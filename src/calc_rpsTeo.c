...
static inline fixed_rowsum_t* rowData_get(row_t row, noRnd_col_t x){
  return & rowData[rowTransform_row_hash(row) + row_size * x];
}

static inline double* probeData_min_get(row_t row, noRnd_col_t x){
...
static void rowData_init(row_t row, noRnd_col_t x){
  ...
  *it = 0.0;
  for(noRnd_col_t i = 1; i < 1ll<<NUM_INS; i++){ // 0 excluded
    *it += transform[calcUtils_intExpandByD(i)] * xor_col(i, x);
  }
}

static fixed_rowsum_t rowData_sumPhase(row_t row, noRnd_col_t x){
...
static double probeData_sumPhase(row_t row, row_t o, noRnd_col_t x){
...
static void probeData_min_init(row_t row, noRnd_col_t x){
  ...
  *it = *it / 2 * ldexp(1.0, - row_count1(row)); // 2 ** - row_count1(row) = multeplicity * 2** - numprobes
  *it = MIN(1, *it);
}

static double probeData_evalMin(row_t row, noRnd_col_t x){
  return *probeData_min_get(row, x);
}


// with multeplicity
static coeff_t toBeSummed(row_t highest_row, noRnd_col_t x){
...
static coeff_t sumProbes(noRnd_col_t x){
