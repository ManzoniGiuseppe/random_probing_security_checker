...
function findCoeffAndSaveResults(){
  ...
  for c in $(seq 0 $maxCoeff) ; do
    ...
    if [ ! -e $out/$c.success ] ; then
      ...
      if [ $status -eq 0 ] ; then
        cat $mainDir/_tmp_out | tail -n 3 | sed 's/^[a-z]*[\t ]*//;s/m/:/' >> $out/$c.success
...
gadgets="${gadgets} vrapsPaper_copy.sage:13:5"
gadgets="${gadgets} otpoePaper_small_add.sage:9:5"
gadgets="${gadgets} otpoePaper_small_mul.sage:5:5"
...
generators="${generators} otpoePaper_copy.py__3:6:6"
generators="${generators} isw_refresh.py__3:9:9"
generators="${generators} otpoePaper_add.py__3:5:5"
generators="${generators} otpoePaper_copy.py__3:5:5"
